...
const Timer:React.FC<TimerProps> = () => {
    // Keep track of the timer value as well as the flag to show/hide the timer
    const [time, setTime] = useState<number>(0);
    const [showTimer, setShowTimer] = useState<boolean>(false);

    /**
     * Formats the given time into a string in hh:mm:ss format.
     * 
    ...
    /**
     * Called when the showTimer state changes.
     * When the showTimer state changes to true, the timer will be displayed and updated
     * every second.
     */
    useEffect(() => {
    ...
    return (
      <div>
        {showTimer ? (
          ...
          <div className="flex items-center p-1 h-8 hover:bg-dark-fill-3 cursor-pointer rounded"
